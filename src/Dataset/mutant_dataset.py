...
class Mutant(Dataset):
    ...
    def generate_protein_graph_evaluation(self):
        ...
        for idx, name in enumerate(tqdm(names)):
            ...
            if not rec_graph:
                ...
                continue
            score_info = self.generate_score_matrix(protein_dir)
            if score_info:
    ...
    def generate_score_matrix(self, protein_dir):
        protein_name = protein_dir.split('/')[-1]
        df = pd.read_table(os.path.join(protein_dir, protein_name + ".tsv"))
        if len(df.columns) == 3:
        ...
        for i in df.index:
            sep = ";"
            if ":" in df.loc[i, 'mutant']:
                sep = ":"
            mutant = df.loc[i, 'mutant'].split(sep)
            ...
            for j in range(len(mutant)):
                ...
                mutant_matrix[j, 2] = self.amino_acids_type.index(
                    mutant[j][-1])
